...
def searchIn(word, sentance):
  ...
  if sentance[x - 1] not in VOIDCHARACTERS:
    if (x + wordlen + 1) < sentancelen and [x + wordlen + 1] in VOIDCHARACTERS:
      return True
  return False
...
async def on_message(message):
  ...
  try:
    if msg.upper() == "!ADD TRIGGER":
      ...
      trigger = (await client.wait_for('message', check=lambda m: m.author == message.author, timeout=60)).content
      await message.channel.send("Enter Result")
      result = (await client.wait_for('message', check=lambda m: m.author == message.author, timeout=60)).content
      await message.channel.send("Enter Type(in/is)")
      type_ = (await client.wait_for('message', check=lambda m: m.author == message.author, timeout=60)).content
      if type_.lower() not in types:
        await message.channel.send("jesus christ learn to follow basic instructions")
        return
      else:
